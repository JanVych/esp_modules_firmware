...
static void _RunCheck(void *pvParameters)
{
    nvs_handle handle;
    size_t len;
    char* string;
    ...
    if (_CheckAppState()) 
    {
        ...
        {
            string = malloc(len);
            nvs_get_str(handle, "ProgramName", string, &len);
            nvs_set_str(handle, "_ProgramName", string);
            nvs_commit(handle);
            free(string);
            nvs_close(handle);
        }
...
static void _ProgramRun()
{
    if(!_isProgramRunning && _programTask == NULL){
        xTaskCreate( program_Main, "program", 20480, NULL, tskIDLE_PRIORITY, &_programTask );
    }
    if(!_isProgramRunning && _programTask != NULL){
